...
class ImageNet(ExtendedVisionDataset):
    ...
    def get_image_data(self, index: int) -> bytes:
        ...
        #else:
        denoised_image_full_path = os.path.join(f"imagenet-{os.environ['NOISE_TYPE']}-denoised", image_relpath)
        with open(image_full_path, mode="rb") as f:
            image_data = f.read()
        with open(denoised_image_full_path, mode="rb") as f_d:
            denoised_image_data = f_d.read()
        return image_data, denoised_image_data
    # def get_image_data(self, index: int) -> bytes:
    #     entries = self._get_entries()
    #     actual_index = entries[index]["actual_index"]
