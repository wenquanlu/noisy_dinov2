...
def gaussian_noise(img: Image.Image, std: float) -> Image.Image:
    ...
    Returns:
        PIL.Image with added Gaussian noise
    """
    x = np.array(img) / 255.0
    noise = np.random.normal(loc=0.0, scale=std, size=x.shape)
    x_noisy = np.clip(x + noise, 0.0, 1.0) * 255
    return Image.fromarray(x_noisy.astype(np.uint8))
...
class MixedDataAugmentationDINO(object):
    def __init__(
        self,
        global_crops_scale,
        local_crops_scale,
    ...
    def __call__(self, image_info):
        image_raw, is_noisy = image_info
        #import sys
        #(image.size == image_d.size, file=sys.stderr)
        output = {}
        if not is_noisy:
            if self.mix_train_single_noise:
                image_d = image_raw
                image = gaussian_noise(image_raw, std=self.mix_train_std)
                #print("single case!!!")
            elif self.mix_train_double_noise:
                image_d = gaussian_noise(image_raw, std=self.mix_train_std)
                image = gaussian_noise(image_raw, std=self.mix_train_std)
        else:
            image_d = image_raw
            image = image_raw
